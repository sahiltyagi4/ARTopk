...
from flexible_compression.miscellaneous.compression import ARVarianceTopKCompress


# Implementation of VAR-Topk compression
class VAR_TopKCompression(object):
    def __init__(self, args):
        ...
        self.final_cr = args.final_cr
        self.epoch_warmup = args.epoch_warmup
        self.compression = ARVarianceTopKCompress(device=self.device, concat_size=self.concat_size,
        ...
        self.gradvar_smooth = helper.EWMAMeter(windowsize=self.windowsize, alpha=self.smoothing)
    ...
    def launch_training(self):
        ...
        for epoch in range(0, self.epochs):
            ...
            for record in self.trainloader:
                ...
                worker_gnorm = torch.norm(concat_tnsr)
                self.gradvar_smooth.smoothendata(worker_gnorm)
                smooth_norm = self.gradvar_smooth.smooth_val()
                concat_tnsr_size = concat_tnsr.size()
                compress_time = None

                worker_gradvars = [torch.FloatTensor([0.]).to(self.device) for _ in range(self.worldsize)]
                dist.all_gather(worker_gradvars, torch.FloatTensor([smooth_norm]).to(self.device))
                # rank corresponding to worker with gradients of max variance
                var_rank = worker_gradvars.index(max(worker_gradvars))

                if self.rank == var_rank:
                    begin = perf_counter_ns()
                    cmprss_vals, cmprss_ixs = self.compression.ar_variance_compress(tensor=concat_tnsr, cr=self.cr)
                    compress_time = (perf_counter_ns() - begin) / mill
                    ix_len = torch.LongTensor([cmprss_ixs.size()[0]])
                    ix_len = ix_len.to(self.device)

                elif self.rank != var_rank:
                    ix_len = torch.LongTensor([0])
                    ix_len = ix_len.to(self.device)

                dist.broadcast(tensor=ix_len, src=var_rank, async_op=self.async_op)
                if self.rank != var_rank:
                    cmprss_ixs = torch.zeros(size=[ix_len.item()], dtype=torch.int64)

                cmprss_ixs = cmprss_ixs.to(self.device)
                begin = perf_counter_ns()
                dist.broadcast(tensor=cmprss_ixs, src=var_rank, async_op=self.async_op)
                torch.cuda.synchronize(self.device)
                bcast_time = (perf_counter_ns() - begin) / mill

                if self.rank != var_rank:
                    cmprss_vals = self.compression.compress_from_indices(tensor=concat_tnsr, ix_tensor=cmprss_ixs)

                # ready for AllReduce collective
                begin = perf_counter_ns()
                ...
                # apply the reduced updates on the reconstructed tensor
                reduced_grads = []
                ...
                logging.info(f'imglog tstep {self.globalstep} epoch {epoch} compute_time {compute_time} ms '
                             f'lr {sched_lr} var_rank {var_rank} bcast_time {bcast_time} '
                             f'allreduce_time {allreduce_time} ms compress_time {compress_time} ms')
...
if __name__ == "__main__":
    ...
    VAR_TopKCompression(args).launch_training()